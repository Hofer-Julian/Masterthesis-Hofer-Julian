...
\end{figure}
As will be shown in section~\ref{sec:fabry-simulation} the chirp parameter $\alpha$ has hardly any measurable influence on the resulting signal.
...
The accumulated group velocity dispersion introduced by the pulse expander is described by
\begin{equation}
\frac{d^2 \Phi}{d \omega^2} = k \beta^2 2 d
\end{equation}
where $d$ is the (adjustable) distance between the grating and focal plane of the lens and $\beta$ depends on the parameters of the grating and the lens.
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\linewidth]{figures/chirp/group-velocity-dispersion}
	\caption{Influence of a positive group velocity dispersion on a Gaussian pulse~\cite{orfanidis_electromagnetic_2002}}
	\label{fig:group-velocity-dispersion}
\end{figure}

As can be seen in figure~\ref{fig:group-velocity-dispersion}, a positive group velocity dispersion has a very similar effect on a Gaussian pulse as a positive chirp.
...
\label{sec:arp}
One way to inverse the \ac{QD} from the ground state to the biexciton state is by exciting it with a transform-limited laser pulse of constant centre frequency, which is equals to half of the ground state biexciton transition frequency.
...
Basically, the goal is to adiabatically excite from ground state to biexciton state without energy-level crossing~\cite{hui_proposal_2008}.
Schemes which provide that, are also stable with respect to intensity changes of the laser field.

In order to calculate the final biexciton occupation, \textcite{glassl_biexciton_2013} assumed a linearly-chirped Gaussian laser pulse as discussed in section~\ref{sec:chirp}.
...
If $E$ could be measured directly, the chirp could be easily estimated, but this is unfortunately not the case.
...
What is possible is sending the laser into an \ac{IAC} which outputs a signal as shown in figure~\ref{fig:imgausschirpwithoutslitwithoutmosaic} and described by equation~\eqref{eq:i-m-integral}.
