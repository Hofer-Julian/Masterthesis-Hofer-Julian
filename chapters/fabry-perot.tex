...
\end{equation}
with the parameters $A, B, C, D$ determined by the optical element transforming the Gaussian beam described by $q_{in}$.\todo{where do this elements come from and which basis is used by this matrix formalism. Without these definitions, these letters mean nothing.}
...
\todo{Make also the left Y axis logarithmic.
	Also: Can you normalize the intensities?}
\begin{figure}[H]
	\centering
...
\todo{The indices are a mess.. try to get it a bit more astethic. phi-FABRI,PEROT, that's far too long...}
\begin{equation}
...
\todo{I think this is not the scanning mode. The scanning mode is when the piezo moves fast back and forth, and not stepwise. Look it up in the literature please. (look at Firefox tab)}
\begin{figure}[H]
	\centering
...
\newpage
\todo{Better:
	4.5 Setup and Measurement
	General stuff
	4.5.1 Scanning mode with fast photodiodes
	4.5.2 Stepwise mode (find better name) with CCD}
...
\todo{1) What APDs, which time resolution
	2) What scanning frequency and why
	3) Show pictures of the measurement itself and discribe features, errors etc.
	5) What time range did you define, how did you configure the correlation hardware.
	4) We did not connect the TTL-Output of the Funky-Generator directly to the Hydraharp, as it uses a different standard (NIM).}
