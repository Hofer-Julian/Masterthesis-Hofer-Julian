...
\chapter{Planning and building a Fabry-Pérot interferometer}
...
Equation~\eqref{eq:e-field-after-fresnel-approximation} resembles the plane wave in equation~\eqref{eq:plain-wave} modulated by $\exp(ik\rho^2/2z)$.
...
The electric and magnetic fields of Gauss modes are transversal to their propagation direction.
These waveforms are called transversal electric and magnetic modes with indices $(m,n)$.
The Gauss mode's fundamental solution is the TEM$_{00}$-Mode, which is the most important one and will therefore be examined in more detail in the rest of this subsection.
...
\begin{equation}
\frac{1}{q(z)}=\frac{z+iz_0}{z^2+z_0^2}=\frac{1}{R(z)}+i\frac{2}{k w^2(z)},
\end{equation}
with new variables $z_0$, $R(z)$ and $w(z)$ being introduced.
...
\begin{equation}
\exp\left(i\frac{k\rho^2}{2q(z)}\right) \rightarrow \exp\left(i\frac{k\rho^2}{2R(z)}\right)\exp\left(-\left(\frac{\rho}{w(z)}\right)^2\right)
\end{equation}
\begin{figure}[H]
	\centering
	...
	{A Gaussian beam near its beam waist.
		Near the center they resemble plane wave fronts, while outside they converge towards spherical wave fronts.
...
\begin{itemize}
	\item \textbf{Evolving radius of curvature} $b(z)$:
	\begin{equation}
	\label{eq:radius-wavefronts}
	b(z)=z(1+(z_0/z)^2)
	\end{equation}
	\item \textbf{Beam waist} $2w_0$:
	\begin{equation}
	\label{eq:beam-waist}
	w_0^2=\lambda z_0/\pi
	\end{equation}
\end{itemize}
The beam waist $2w_0$ or beam radius $w_0$ describes the smallest  beam cross section at $z=0$.
If the wave propagates inside a medium with the refractive index $n$, $\lambda$ has to be replaced with $\lambda/n$.
The cross section of the beam waist  is then $w_0^2=\lambda z_0/(\pi n)$.

A Gaussian beam can be completely characterized at every point $z$ on the beam axis either with the parameter couple $(w_0, z_0)$ or alternatively with the real and imaginary part of $q(z)$.
...
The ABCD-matrix transforms rays at distance $x_1$ from the optical axis at an angle $\theta$ according to
\begin{equation}
\begin{pmatrix} x_2 \\ \theta_2 \end{pmatrix} = \begin{pmatrix} A & B \\ C & D \end{pmatrix}\begin{pmatrix} x_1 \\ \theta_2 \end{pmatrix}
\end{equation}
...
\end{equation}
Additionally, the electric field $\mathbf{E}$ of equation~\eqref{eq:e-field-after-fresnel-approximation} is inserted into the Helmholtz equation.
...
According to equation~\eqref{eq:photon-decay} the number of photons decays with the photon-loss time $\tau_c$.
...
Therefore, the field $E_{circ}$ can be calculated from $E_{laun}$ by
\begin{equation}
...
The Airy finesse is the determining property when it comes to the spectral resolution of the \ac{FPI}. This can be made visible by comparing its meaning with the Taylor criterion for the resolution of two adjacent peaks.
The Taylor criterion states that two spectral lines are resolvable when the separation of the maxima is greater than their \ac{FWHM}.
...
As sketched in figure~\ref{fig:excitation-of-transverse-mode}, an input beam $E_{in}$ propagates into the resonator with curved mirrors and potentially excites its modes $E_{mn}(x,y)$, where $m$, $n$ are the transverse mode integers of the Gaussian beam of the optical resonator.
...
\end{equation}
From equation~\eqref{eq:coupling-efficiency-second}, it can be deduced that for an input beam with the \textit{same} spatial dependency as the mode to be excited
\begin{equation}
E_{in}(x,y) \sim E_{mn}(x,y).
\end{equation}
Under this condition, all of the incident power goes into $E_{mn}$, i.e. $\eta_{mn}=1$ and all other $\eta_{m'n'}$ are zero.
...
In practice, additional measures are necessary such as matching the radius of curvature by Gaussian beam focusing.
...
In order to match the radius of curvature of the incoming Gaussian beam with the radius of curvature of the resonator a lens is inserted as depicted in figure~\ref{fig:gaussian-beam-focusing}. Light with a beam radius of $w_{01}$ gets focused into the resonator. Transformations by thin lenses can be described with the ray transfer matrix introduced in subsection~\ref{subsec:gaussian-beam}:
\begin{equation}
\begin{pmatrix}
A & B \\
C & D
...
\begin{equation}
q_{in} = z + i \frac{\pi n w_{0,in}^2}{\lambda} \qquad \mathrm{and} \qquad
q_{out} = \frac{q_{in}}{q_{in} \cdot \frac{-1}{f} + 1} =  z + i \frac{\pi n w_{0,out}^2}{\lambda}
\end{equation}
with $n\approx 1$ for air. Together with equation~\eqref{eq:beam-waist} the following relation can be deduced
\begin{equation}
\label{eq:w-o-out}
w_{0,out}^2 = \frac{w_{0,in}^2}{\left(1-\frac{z}{f}\right)^2+\left(\frac{\pi w_{0,in}}{\lambda f}\right)^2}.
\end{equation}
The radii of curvature have to match in order to ensure that the beam returns the same way after being reflected by one mirror.
For given mirrors (described by $b_{mirror}$) and lens (described by $f$) the input beam waist has to be adjusted according to equation~\eqref{eq:radius-wavefronts} and \eqref{eq:beam-waist}
\begin{align}
b_{mirror} \stackrel{!}{=} b_{gauss}(z= l/2)\\
b_{mirror} \stackrel{!}{=} \frac{l}{2} \left(1+\left(\frac{2z_{0,out}}{l}\right)^2\right) \\
\label{eq:R-mirror-1}
b_{mirror} \stackrel{!}{=} \frac{l}{2} \left(1+\left(\frac{2w_{0,out}^2 \pi}{l \lambda}\right)^2\right).
\end{align}
Inserting equation~\eqref{eq:R-mirror-1} into equation~\eqref{eq:w-o-out} results in the condition for mode matching
\begin{equation}
\label{eq:R-mirror-2}
b_{mirror} = \frac{l}{2} \left(1+\left(\frac{2 w_{0,in}^2 \pi}{\left(\left(1-\frac{z}{f}\right)^2+\left(\frac{\pi w_{0,in}}{\lambda f}\right)^2\right)l \lambda}\right)^2\right).
\end{equation}

One way to further suppress higher modes is \textit{spatial filtering}. It can be seen in figure~\ref{fig:gauss-modes-higher-order} that the effective area of a mode increases with its order $(m, n)$. Figure~\ref{fig:spatial-filtering-of-gauss-modes} shows one way to suppress higher modes consisting of a focusing lens and a pinhole whose diameter matches the one of the TEM$_{00}$ mode.
...
\begin{figure}[H]
	...
	\caption[Spatial filtering of Gauss modes.]{Spatial filtering of Gauss modes.
		...
		A TEM$_{01}$ beam is displayed as an example of how higher modes can be suppressed by an aperture.~\cite{meschede_optik_2008}}
...
If the incoming beam would represent a perfect TEM$_{00}$ mode, spatial filtering would not be necessary and mode matching would be easy.
...
By giving up the ability to choose different free spectral ranges with a given pair of mirrors, the confocal setup liberates from mode matching considerations as the cavity is mode degenerate, i.e. the frequencies of certain axial and transverse cavity modes are the same.
...
\end{align}
If mode matching is not performed, it can be assumed that the incoming beam consists of an approximately equal number of even and odd transverse modes.
...
In absence of spectral broadening due to charge noise, the zero-phonon line is described by a Cauchy distribution
\begin{equation}
...
The phonon side band can be described by a Gauss distribution
\begin{equation}
...
As discussed the mirror distance $l$ is adjusted to a value which depends on whether a resolved \ac{ZPL} or a resolved \ac{PSB} is desired.
...
\begin{figure}[H]
	...
	\caption{The exciton emission $\Phi_{dot}(E)$ (red) compared to the transmission of the FPI modes $A'_{trans}$ (blue) of a FPI with a mirror distance $l$ suitable to resolve either (a) the ZPL or (b) the PSB.}
	\label{fig:simulation-comparison-dot-fabry-perot-modes}
\end{figure}

The \ac{FPI} is scanned by continuously varying the mirror distance $l$ with a certain stepsize $\Delta l$ and measuring the output-photon-flux every time.
This process is sketched in figure~\ref{fig:simulation-comparison-dot-fabry-perot-sweep}.
Similar to figure~\ref{fig:simulation-comparison-dot-fabry-perot-modes}a the \ac{PSB} can not be seen, as it is too weak to be visible in the linear scale.
...
The first version shown in figure~\ref{fig:old} featured planar (diameter $d=\SI{25.4}{\milli \meter}$) mirrors, while their distance was coarsely tunable with an adjustable platform and finely tunable with a piezoelectrical actuator.
...
The mirror-distances in figure~\ref{fig:topview} are in the order of sub-micrometer.
...
\begin{figure}[H]
	...
	\begin{subfigure}[b]{0.48\textwidth}
		...
		\caption{FPI on adjustable platform and finely tunable with a piezoelectrical actuator and planar mirrors of diameter $d=\SI{25.4}{\milli \meter}$.\\}
		\label{fig:old}
	\end{subfigure}%
	...
	\begin{subfigure}[b]{0.48\textwidth}
		...
		\caption{FPI on molybdenum plate, coarsely tunable with elongated hole and finely tunable with a piezoelectrical actuator and planar mirrors of diameter of $d=\SI{12.7}{\milli \meter}$.}
		\label{fig:new}
	\end{subfigure}
...
Molybdenum has a low thermal expansion coefficient, which is advantageous because of the \ac{FPI}'s sensitivity to mirror-distance-changes. 
...
\begin{figure}[H]
	...
	\begin{subfigure}[b]{0.48\textwidth}
		...
		\caption{Top view of FPI with planar mirrors.}
		\label{fig:topview}
	\end{subfigure}%
	~ % An dieser Stelle kann ein zusätzlicher Zwischenraum eingebunden werden: ~, \quad, \qquad, \hfill usw.
	% Eine leere Zeile erzwingt, dass die zweite Grafik darunter erscheint.
	\begin{subfigure}[b]{0.48\textwidth}
		...
		\caption{Casing made of aluminium, built to limit thermal fluctuations of the \acp{FPI} environment.}
		\label{fig:casing}
	\end{subfigure}
...
\begin{figure}[H]
	...
	\caption{Live avalanche photodiode setup using a function generator driving the piezo actuator of the FPI and the sync input of the correlation hardware.
	APDs are used because of their speed, allowing to live-adjusting the FPI-parameters.}
	\label{fig:live-apd-setup}
\end{figure}

Before the scanning \ac{FPI} can be used to measure \ac{QD} emission, it has to be aligned first usually with a (HeNe) laser.
...
A function generator is used to drive the piezoelectrical actuator and the correlation hardware (here: Picoquant Hydraharp 400) correlates the measurements of the \ac{APD} with the TTL-output of the function generator in the time domain.
In order to do that the TTL-output has to be converted to the NIM standard beforehand, as this is the only one our correlation hardware accepts.
TTL interprets \SI{0}{\volt} as LOW and \SI{5}{\volt} as HIGH and NIM interprets \SI{0}{\volt} as LOW and negative values not lower than \SI{-1}{\volt} as HIGH.
The conversion therefore involves inversion and attenuation of the signal.
...
The scanning frequency is \SI{5}{\hertz} and the scanning distance is \SI{1}{\micro \meter}.
The measured finesse is approximately $5$ and the free spectral range corresponds to \SI{280}{\nano \meter}.
...
In order to obtain the line shape of the input signal, the \ac{FPI} is used in the scanning mode.
The \ac{FPI} is aligned with \acp{APD} and a HeNe laser as the former offers direct feedback and the latter emits visible light simplifying the procedure.
Afterwards, the signal of interest will be measured, in our case \ac{QD} emission.
The piezoelectric actuator moves step-wise one of the mirrors and after every step the output is recorded with the \ac{CCD}.
...
This involves positioning the laser beam so that it goes straight and centred through the \ac{FPI} mirrors. 
Afterwards, the \ac{FPI} mirrors are adjusted so that they are parallel and a single laser beam emerges as shown in figure~\ref{fig:align-fpi}.
...
\begin{figure}[H]
	...
	\begin{subfigure}[b]{0.48\textwidth}
		\centering
		\includegraphics[width=0.95\textwidth]{figures/fabry-perot/setup/setup-higher-modes-1}
		\caption{}
		\label{fig:setup-higher-modes-1}
	\end{subfigure}%
	~ % An dieser Stelle kann ein zusätzlicher Zwischenraum eingebunden werden: ~, \quad, \qquad, \hfill usw.
	% Eine leere Zeile erzwingt, dass die zweite Grafik darunter erscheint.
	\begin{subfigure}[b]{0.48\textwidth}
		\centering
		\includegraphics[width=0.95\textwidth]{figures/fabry-perot/setup/setup-higher-modes-2}
		\caption{}
		\label{fig:setup-higher-modes-2}
	\end{subfigure}
...
To counteract them, a pinhole is inserted and adjusted, which results in the measurement visible in figure~\ref{fig:measurement-fabry-perot-hene}b.
The intensity with and without \ac{FPI} were compared in order to calculate the \ac{FPI}'s transmission at the peak of its modes, leading to $T_{peak}=\SI{5}{\percent}$.
With figure~\ref{fig:measurement-fabry-perot-hene}b the finesse can be calculated.
As the mirror reflectivities $R=R_1=R_2=\SI{97}{\percent}$, the mirror reflectivity finesse is $F=155$.
The measured finesse is $45$ and obviously much lower.
This is because the measured finesse $F_t$ also depends on the mirror surface quality finesse $F_q$ and the finesse due to illumination conditions (beam alignment and diameter) of the mirrors $F_i$ as described by~\cite{noauthor_scanning_nodate}
\begin{equation}
1/F_t = \left[(1/F)^2+(1/F_q)^2+(1/F_i)^2\right]^{1/2}.
\end{equation}
...
As can be seen in figure~\ref{fig:measurement-fabry-perot-hene} the arrangement is stable enough that Cauchy peaks are symmetric and not distorted and fine details like the higher modes are resolvable.
...
As a change of $\lambda/2$ in the mirror separation corresponds to a mode shift of one free spectral range, the corresponding \ac{FPI} mode shift can be estimated.
...
\begin{figure}[H]
	...
	\caption{Polarization map of exciton-groundstate decay emission of QD in sample AS208.
	The peak to peak amplitude corresponds to the fine structure splitting.}
	\label{fig:measurement-fabry-perot-dot-exciton-pol-map}
\end{figure}

Set up like this, the GaAs \ac{QD} sample AS208 was measured.
First, a polarization map of an exciton was recorded in order to determine the \ac{FSS}.
This works by putting a $\lambda/2$-plate followed by a fixed linear polarizer between the \ac{QD} and the spectrometer.
...
\begin{figure}[H]
	...
	\caption{Fine-structure measurement of exciton decay emission of a QD in sample AS208.
		 It is measured by passing the QD emission through a FPI and sweeping the mirror distance.
		The ground mirror distance is  $l \approx \SI{2}{\milli \meter}$.}
	\label{fig:measurement-fabry-perot-dot-biexciton-fss}
\end{figure}

Afterwards, the output of the \ac{QD} through the \ac{FPI} was measured while the mirror distance was stepwisely changed as shown in figure~\ref{fig:measurement-fabry-perot-dot-biexciton-fss}.
...
In contrast to measurements with the HeNe-laser in figure~\ref{fig:measurement-fabry-perot-hene}, the higher transverse modes are not visible in figure~\ref{fig:measurement-fabry-perot-dot-biexciton-fss}.
This is because there are two lines overlapping and the linewidth of a single one is already broader than the HeNe laser linewidth.
Therefore, the  higher transverse modes are overlaid by the fundamental mode.

From the performed measurements and the considerations about the stability, it can be deduced that with mirrors of correct geometry fine details of \ac{QD} emission can be resolved.
In order to use this in daily measurements however, mirrors with different curvature radii will be used in order to set up the \ac{FPI} in order to satisfy the confocal conditions.
This would liberate from time-consuming and error-prone mode-matching and spatial filtering.
