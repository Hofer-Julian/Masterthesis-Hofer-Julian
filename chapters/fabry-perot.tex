...
\begin{itemize}
	...
	R(z)=z(1+(z_0/z)^2)
	\end{equation}
	At the beam waist the curvature of the wavefronts is zero as well as for $z\rightarrow \infty$
...
In order to match the radius of curvature of the incoming Gaussian beam with the radius of curvature of the resonator a lens is inserted as depicted in figure~\ref{fig:gaussian-beam-focusing}. Light with a beam waist of $\omega_{01}$ gets focused into the resonator. Thin lenses can be described with the ABCD-rule introduced in subsection~\ref{subsec:gaussian-beam}:
\begin{equation}
\begin{pmatrix}
A & B \\
C & D
...
The incoming beam described by $q_{in}$ is transformed by the lens to an beam described by $q_{out}$ according to equation~\ref{eq:beam-waist} and \eqref{eq:ABCD-rule}
\begin{equation}
...
\end{equation}
The radii of curvature have to match, for given mirrors ($R_{mirror}$) and lens ($f$) the input beam waist has to be adjusted according to equation~\eqref{eq:radius-wavefronts} and \eqref{eq:beam-waist} 
\begin{align}
...
\end{align}
inserting equation~\eqref{eq:w-o-out}
\begin{equation}
