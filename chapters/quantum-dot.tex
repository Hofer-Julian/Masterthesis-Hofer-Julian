...
$\Gamma$ describes the transition probability per unit time from an initial state $\ket{i}$ to a final state $\ket{f}$ and it can be calculated by Fermi's golden rule if the the transition occurs because of a weak perturbation.
In the dipole approximation it can be described by
\begin{equation}
...
It states that electronic transition between ground and excited state is much faster than the motion in the lattice as the photons carry nearly no momentum.
